...
export class GGBPlotter {
    ...
    if (page) {
      ...
      if (this.poolOpts.ggb === "local") {
        const dir = path.resolve(
          __dirname,
          "../geogebra-math-apps-bundle/GeoGebra/HTML5/5.0/GeoGebra.html"
        );
        url = "file://" + dir;
      } else {
  ...
  async evalGGBScript(ggbScript: string[], width?: number, height?: number) {
    ...
    if (ggbScript && ggbScript.length) {
      await page.evaluate(
        (x) => window.ggbApplet.evalCommand(x),
        ggbScript.join("\n")
      );
    }
  }
  ...
  async exportPNG64(alpha?: boolean, dpi?: number): Promise<string> {
    ...
    const out = await page.evaluate(
      (alpha, dpi) => window.ggbApplet.getPNGBase64(1, alpha, dpi || 300),
      alpha,
      dpi
    );
