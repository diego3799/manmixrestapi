...
export class GGBPool {
  ...
  async ready(): Promise<GGBPool> {
    ...
    const promises = new Array<Promise<puppeteer.BrowserContext>>(
      this.opts.plotters
    );
    ...
    // file:///home/diego3799/Documents/node-geogebra/geogebra-math-apps-bundle/Geogebra/HTML5/5.0/GeoGebra.html
    // file:///home/diego3799/Documents/node-geogebra/geogebra-math-apps-bundle/GeoGebra/HTML5/5.0/GeoGebra.html
    let url;
    if (this.opts.ggb === "local") {
      ...
      url = "http://127.0.0.1:5501/geogebra-math-apps-bundle/GeoGebra/HTML5/5.0/GeoGebra.html";
    }
    ...
    DEBUG &&
      console.log("https://www.geogebra.org/classic have loaded in all pages");

    // Wait for ... ggbApplet injected
    promises3 = new Array(this.opts.plotters);
    for (var i = 0; i < this.opts.plotters; i++) {
      promises3[i] = this.availablePages[i].waitForFunction(
    ...
    promises3 = new Array(this.opts.plotters);
    for (var i = 0; i < this.opts.plotters; i++) {
      promises3[i] = this.availablePages[i].evaluate(
    ...
    this.availableWorkers = this.availablePages.map(
      (p, i) => new GGBPlotter(i + 1, p, this.releasedEmitter)
    );
